...
function removePunctuation(string) {//remove all non-letters from the input string
  ...
  strToSeq = strToSeq.filter(function(k) {
    return k.match(lettersOnly)
  })
}
...
function execute() {
  ...
  for (var i = 0; i < strToSeq.length; i++) {
    numericalIndex(strToSeq[i])
  };//at this point, the sequence of integers has been generated 
  for (var i = 0; i < indeces.length; i++) {//this sub-function addresses all values in the sequence larger than 8
    if (indeces[i] <= 8) {
  ...
  for (var i = 0; i < reducedIndeces.length; i++) { //translate the sequence of integers to a sequence of scale degrees
...
function playOsc(x) {//iterate through the scale degree sequence and play a new oscillator at the specified pitch once every x milliseconds
  ...
  playTimer = setInterval(function(){
    if (noteNo == reducedIndeces.length) {//if at end of sequence, terminate iteration
